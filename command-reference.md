...
| `--test-runs-all` | | flag | false | Backup all test runs regardless of date (overrides --test-runs-days) |
...
| `--test-runs-days` | | int | 90 | Number of days of test run history to backup (default: 90 days) |
| `--include-all-runs` | | flag | false | Backup all test runs regardless of date (overrides `--test-runs-days`) |
...
```bash
adobackup.exe testplans-backup --include-all-runs -v
```
...
- Use `--test-runs-days 0` or `--include-all-runs` for complete history (may take long time)
...
| `--include-runs` | | flag | false | Also restore test runs (in addition to plans and suites) |
| `--dry-run` | | flag | false | Preview changes without applying them |
...
- **Test Runs** (optional): Test run history with results (requires `--include-runs`)
...
**Restore with test runs:**
```bash
adobackup.exe testplans-restore --include-runs -v
```
...
**Notes:**
- Test plans and suites are always restored; test runs are optional (`--include-runs`)
...
- ID mapping files enable re-running restore idempotently
