...
**Example 2: Skip large/static resources**
```bash
# Skip Git if using separate Git backup solution
adobackup.exe backup-all --skip-git -v
```
...
adobackup.exe backup-all -v 2>&1 | 
  ...
  Send-ToSplunk -Index "adobackups"
```
...
steps:
  ...
  - powershell: |
      ...
      robocopy "$(Backup.Root)" $archivePath /MIR /Z /R:3
