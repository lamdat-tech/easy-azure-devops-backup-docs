...
Yes! Use skip flags to exclude resources:

```bash
# Backup only Git repositories
adobackup.exe backup-all --skip-builds --skip-workitems --skip-variables --skip-queries

# Backup only work items
adobackup.exe backup-all --skip-git --skip-builds --skip-variables --skip-queries
```
